...
def test_correct_return(users, channel_ids):
    ...
    payload = requests.post(url + "channel/addowner/v1", json={
...
def test_made_owner(users, channel_ids):
    '''
    ...
    for owner in list_owners:
        if owner["u_id"] == users[1]["auth_user_id"]:
            is_owner = True
...
def test_invalid_channel(users):
    ...
    '''
    assert requests.post(url + "channel/addowner/v1", json={
...
def test_already_owner(users, channel_ids):
    '''

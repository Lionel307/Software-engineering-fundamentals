...
def test_correct_return(users, dm_id_1):
    ...
    '''
    output = requests.post(url + "dm/leave/v1", json={
...
def test_left_successfully(users, dm_id_1, dm_id_2):
    '''
    A test which checks if the user has successfully left the DM and other dms are unchanged
    '''
    ...
    assert dm_left == True and other_dm_present == True
...
def test_invalid_dm(users, dm_id_1):
    ...
    assert requests.post(url + "dm/leave/v1", json={
        "token": users[1]['token'], "dm_id": dm_id_1 + 1
    }).status_code == 400

    assert requests.post(url + "dm/leave/v1", json={
        "token": users[1]['token'], "dm_id": dm_id_1 + 2
    }).status_code == 400

    assert requests.post(url + "dm/leave/v1", json={
        "token": users[1]['token'], "dm_id": dm_id_1 + 3
    }).status_code == 400
...
def test_auth_not_member(users, dm_id_1):
    ...
    assert requests.post(url + "dm/leave/v1", json={
        "token": new_user['token'], "dm_id": dm_id_1
    }).status_code == 403
...
def test_invalid_token(users, dm_id_1):
    ...
    assert requests.post(
        url + "dm/leave/v1", json={"token": users[0]["token"], "dm_id": dm_id_1
    }).status_code == 403

    assert requests.post(
        url + "dm/leave/v1", json={"token": "invalid token", "dm_id": dm_id_1
    }).status_code == 403

    assert requests.post(
        url + "dm/leave/v1", json={"token": 1, "dm_id": dm_id_1
    }).status_code == 403

    assert requests.post(
        url + "dm/leave/v1", json={"token": [1, 2, 3], "dm_id": dm_id_1
    }).status_code == 403
